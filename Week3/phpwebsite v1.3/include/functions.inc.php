...
function createreservation($conn, $vehicle, $timein, $timeout, $usersId) {
    $sql = "INSERT INTO reservation (reserveVehicle, reserveEnter, reserveExit, reserveUser) VALUES (?, ?, ?, ?);";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)){
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $vehicle, $timein, $timeout, $usersId);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
